...
export default function BlogEdit({ post, categories, portfolios = [] }) {
    ...
    const { data, setData, post: submitPost, processing, errors } = useForm({
        ...
        _method: 'PUT',
        // Enterprise fields
        event_type: post.event_type || 'meeting',
        event_scope: post.event_scope || 'tehsil',
        start_date: post.start_date || '',
        end_date: post.end_date || '',
        venue: post.venue || '',
        organizer_portfolio_id: post.organizer_portfolio_id || '',
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        submitPost(route(`${prefix}.blog.update`, post.id), {
            forceFormData: true,
        });
    };
                    ...
                    <div className="md:col-span-2">
                        ...
                        <p className="text-xs text-gray-500 mt-1 ml-1">
                            {data.target_audience === 'all_members'
                                ? 'Invite everyone in your jurisdiction.'
        ...
        <article className="bg-white rounded-3xl shadow-xl overflow-hidden animate-in fade-in duration-300 border border-gray-200">
            ...
            <div className="p-8 md:p-12">
                ...
                <div className="flex flex-wrap gap-4 mb-6 text-sm font-bold text-gray-500 border-b pb-4">
                    ...
                    <span>•</span>
                    <span>Ref: {post.id || 'NEW'}</span>
                    <span>•</span>
                    <span>{data.publish_date ? new Date(data.publish_date).toLocaleDateString() : 'Date TBA'}</span>
                </div>
    ...
    return (
        <AuthenticatedLayout user={auth.user} header={`Edit ${data.category.charAt(0).toUpperCase() + data.category.slice(1)}`}>
            <Head title={`Edit ${data.category}`} />
                                ...
                                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 sticky top-6">
                                    ...
                                    <div className="space-y-4 mb-6">
                                        ...
                                        <div>
                                            ...
                                            <input
                                                type="datetime-local"
                                                value={data.publish_date ? data.publish_date.slice(0, 16) : ''}
                                ...
                                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                    ...
                                    <div className="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center hover:border-black transition cursor-pointer relative bg-gray-50">
                                        ...
                                        <label className="cursor-pointer">
                                            {data.featured_image ? (
                                                <p className="text-green-600 font-medium">✓ New Selected</p>
                                            ) : (
                                                <p className="text-gray-500">📤 {post.featured_image ? 'Replace Image' : 'Upload Image'}</p>
                                            )}
                                        </label>
                                    </div>
                                    {post.featured_image && !data.featured_image && (
                                        <div className="mt-4">
                                            <img src={`/storage/${post.featured_image}`} alt="Current" className="rounded-lg w-full h-32 object-cover mb-3" />
                                            <button
                                                type="button"
                                                onClick={() => {
                                                    if (confirm('Are you sure you want to remove this featured image?')) {
                                                        router.delete(route(`${prefix}.blog.remove-image`, post.id), {
                                                            preserveScroll: true,
                                                            onSuccess: () => {
                                                ...
                                                className="w-full py-2 px-4 bg-red-50 text-red-600 border border-red-200 rounded-lg hover:bg-red-100 transition font-medium text-sm"
                                            >
